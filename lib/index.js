...
function updateSidebar (filesObj) {
    ...
    for (let i = 0; i < categoryKeys.length; i++) {
        ...
        html += sectionTemplate(categoryKey, categories[categoryKey].length, list);
    }

    // read file and replace object before writing?

    fs.writeFile('_Sidebar.md', html, (err) => {
...
// TODO: highlight text on link
        ...
        if (list.length) {
            ...
            list = sectionTemplate('References', totalCount, list);
...
function usedby (fileObj, filesObj, prefix) {
    ...
    if (tester.test(str) && fileObj.usedby.length) {
        ...
        if (list.length) {
            // list = `## Used by  \n\n${list}  \n\n<br />`;
            list = sectionTemplate('Used by', fileObj.usedby.length, list);
            str = replaceContent(str, prefix, list);
        }
    }
...
function toc (fileObj, filesObj, prefix) {
    ...
    if (tester.test(str)) {
        ...
        if (list.length) {
            ...
            list = sectionTemplate('Table of contents', false, list)
            str = replaceContent(str, prefix, list);
        }
    }
...
function getBacklinks (fileObj, filesObj) {
    const files = Object.keys(filesObj).map(key => {
        return {
            title: filesObj[key].name.replace('.md', '').replace(/-/g, ' '),
            name: key
        }
    });
...
function collectDependencies (fileObj, filesObj) {
    ...
    lineByLine(fileObj.content.updated, (line, i) => {
        ...
        if (depsFound) {
            ...
            const name = line.match(/\[\[(.*)\]\]/)[1].replace(/ /g, '-') + '.md';
...
function readFilesOnce (filesObj) {
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        waitForIt.push(new Promise((resolve, reject) => {
            fs.readFile(filesObj[key].name, (err, data) => {
                if (err) console.log(err);
...
function sectionTemplate (title, count, content) {
    count = count ? ` (${count})` : '';

    return `
<details>
  <summary><strong>${title}</strong>${count}</summary><br />
