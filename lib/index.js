...
// TODO: highlight text on link
                ...
                fileObj.backlinks[key].lines.forEach(line => {
                    ...
                    lines.forEach((backlink, i) => {
                        ...
                        if (i < (lines.length - 1)) {
                            let before = backlink.slice(charCount * -1);
                            let after = lines[i+1].substring(0, charCount);
                            
                            before = before.length < charCount ? before : '...' + before.trim();
                            after = after.length < charCount ? after : after.trim() + '...';
                            
                            let newLine = `${before} ***${fileObj.title}*** ${after}`;
                            let matchText = newLine.replace(/(\*\*\*|\.\.\.)/g, '').replace(/\s+/g, ' ');
                            
                            console.log(`${fileObj.backlinks[key].name.replace('.md', '')}#:~:text=${matchText}`);

                            backlinkList += `- <a href="${fileObj.backlinks[key].name.replace('.md', '').replace(/\s+/g, ' ')}#:~:text=${matchText}">${newLine.replace(/(\[\[|]])/g, '')}</a>\n`;
                        }
                    });
