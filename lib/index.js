...
function collectDependencies (fileObj, filesObj) {
    ...
    // TODO: split at Dependencies to reduce lines searched?
...
function recursiveReaddir (path, fileList = []) {
    const waitForIt = [];
    const ignoreList = ['node_modules', 'README.md', 'Home.md'];
...
function sectionTemplate (config, content) {
    ...
    const open = String(config.open) === 'true' ? ` open="true"` : '';
    const reduce = String(config.reduce) === 'true' ? '' : '<br />';

    return `
<details${open}>
  <summary><strong>${config.title}</strong>${count}</summary>${reduce}
